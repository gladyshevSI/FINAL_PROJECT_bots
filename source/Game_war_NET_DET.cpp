...
fast_game = false;
bool visual = true;
bool who = false; //\F0\E5\F9\E0\E5\F2, \E1\F3\E4\E5\F2 \EB\E8 \F2\F3\F2 \F0\F3\F7\ED\EE\E5 \F3\EF\F0\E0\E2\EB\E5\ED\E8\E5 \E8\EB\E8 \ED\E5\F2.
...
//çäåñü ÿ óïðàâëÿåò áîò! NO_HUMAN!
        ...
        for (Bbot = Bots_life.begin(); Bbot != Bots_life.end();Bbot++){
            if (!(*Bbot)->life || (*Bbot)->lives <= 0) { (*Bbot)->life = false;
                /*random_rect(*Bbot);
            ...
            if ((*Bbot)->life) {
                if ((*Bbot)->control()) { bullets.push_back(new Bullet((*Bbot)->rect.left, (*Bbot)->rect.top, t, /*speed_bul*/cos((*Bbot)->da)/2, /*speed_bul*/sin((*Bbot)->da)/2, *Bbot));}
...
//cout << "-3-" << endl;
        ...
        ////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
        //cout << "========================================" << endl;
        vector<DETBot> opponents;
//cout << "-4-" << endl;
        for(const auto& i : Bots_life)
        {
       //cout << "C Bots_life" << endl;
            if(i->life)
            {
                DETBot new_bot(t, i->rect.left/16, i->rect.top/16, 0);
                //cout << i->rect.left << " " << i->rect.top << endl;
                opponents.push_back(new_bot);
            }     
        }

        cout << opponents.size() << endl;
//cout << "-5-" << endl;
        for(auto& i : detbots)
        {
        //cout << "C detbots" << endl;
        ...
        //cout <<"- " << to_x << " " << to_y << endl;
            if(!(to_x == 0 && to_y == 0) && i.get_perez() > 70)
            {
                ...
                bullets.push_back(new Bullet(i.rect.left, i.rect.top, t, dx/2, dy/2, i.get_id()));
 ...
 //cout << "-6-" << endl;
        ...
        for (bul1 = bullets.begin(); bul1 != bullets.end(); bul1++){
            for (it_DETBot = detbots.begin(); it_DETBot != detbots.end(); it_DETBot++){
                //cout << "PULA + DETBot" << endl;
                if ( (*bul1)->owner_DETBot == -1 &&
                    (*bul1)->rect.intersects((*it_DETBot).rect) &&
                    (*bul1)->life && (*it_DETBot).life)
                {
                    (*bul1)->life = false;
        ...
        for (bul = bullets.begin(); bul != bullets.end(); bul++){
            for (bul1 = bullets.begin(); bul1 != bullets.end(); bul1++){
                if (bul1 != bul && (*bul)->rect.intersects((*bul1)->rect) && (*bul)->life && (*bul1)->life) {
                    ...
                    if ((*bul)->owner  != nullptr) {(*bul)->owner->lives  += lives_for_bul;}
                    if ((*bul1)->owner != nullptr) {(*bul1)->owner->lives += lives_for_bul;}
                }
            }
		}
...
// stolknovenie PULA + BOT
        ...
        for (bul1 = bullets.begin(); bul1 != bullets.end(); bul1++){
            for (Bbot = Bots_life.begin(); Bbot != Bots_life.end(); Bbot++){
                //cout << "PULA + DETBot" << endl;
                if ( (*bul1)->owner_DETBot != -1 &&
                    (*bul1)->rect.intersects((*Bbot)->rect) &&
                    (*bul1)->life && (*Bbot)->life)
                {
                    (*bul1)->life = false;
                    (*Bbot)->life = false;
        ...
        for (bul1 = bullets.begin(); bul1 != bullets.end(); bul1++){
            for (Bbot = Bots_life.begin(); Bbot != Bots_life.end(); Bbot++){
                if ((*bul1)->rect.intersects((*Bbot)->rect) && (*bul1)->life && (*Bbot)->life && ((*bul1)->owner != (*Bbot))) {
                    ...
                    if ((*Bbot)->lives <= 0) {
                        (*Bbot)->life = false;
                        if ((*bul1)->owner != nullptr) {(*Bbot)->killer = (*bul1)->owner;}
                    }
                    if ((*bul1)->owner != nullptr) {
...
//printf("C398\n");
  ...
  //  printf("C420\n");
            ...
            //cout << "#" << endl;
            for (Bbot = Bots_life.begin(); Bbot != Bots_life.end(); Bbot++) { if ((*Bbot)->life) {window.draw((*Bbot)->sprite); }}
            //cout << "$" << endl;
